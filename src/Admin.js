...
export default function Admin({ user, role, onSignOut }) {
  ...
  const [filterLevel, setFilterLevel] = useState("all"); // all, ปวช., ปวส.
        ...
        if (u.borrows && u.borrows.length > 0) {
          u.borrows.forEach((b) => {
            borrowsList.push({
              ...
              level: u.level || "-",
              userId: u.id,
              status: b.returnDate ? "returned" : "borrowed", // เพิ่มสถานะ
            });
          });
        }
      });
  ...
  // ฟังก์ชันเปลี่ยน Role (เฉพาะ Admin)
  const changeRole = async (userId, newRole) => {
    if (!window.confirm(`ยืนยันเปลี่ยน Role เป็น ${newRole}?`)) return;
    try {
  ...
  const addBook = async () => {
    ...
    try {
      ...
      await setDoc(doc(collection(db, "books")), {
        name: newBook.category, // ใช้ชื่อหมวดหมู่แทน
  ...
  const getFilteredBorrows = () => {
    ...
    // กรองตามคำค้นหา (ชื่อหรือรหัสนักศึกษา)
    if (searchText) {
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        <div>
          ...
          <p style={s.subtitle}>ผู้ใช้: {user?.email} | Role: admin</p>
        </div>
      ...
      <div style={s.content}>
                ...
                {users.map((u) => (
                  <div key={u.id} style={s.row}>
                    <div style={s.userInfo}>
                      ...
                      {/* actions ย้ายมาข้างล่าง userInfo เพื่อไม่ล้นบน mobile */}
                      <div style={s.actionsBelow}>
                        <select
                          value={u.role || "user"}
        ...
        {/* TAB: Borrows - พร้อมฟีเจอร์ค้นหาและกรอง */}
        {activeTab === "borrows" && (
          <div>
...
const s = {
  ...
  // ✅ actions บน users tab — วางข้างล่าง userInfo เพื่อไม่ล้น
  actionsBelow: {
    display: "flex",
    gap: 10,
    alignItems: "center",
